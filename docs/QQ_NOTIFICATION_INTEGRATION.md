...
自动测试仅使用 `127.0.0.1` 上的 HTTP/WebSocket 模拟服务和虚构凭据。真实 QQ 接口只在手动操作工具时使用。原线程中用户已反馈连接成功；FRLG 副本的真实收发仍需单独记录结果。

2026-09-20 本轮验证：Python 3.12.10 / PySide6 6.11.2，客户端 13 项离线测试和服务层 4 项离线测试全部通过；主窗口入口、设置页和教程页在 offscreen 环境创建并截图，12 张教程图全部加载。测试使用临时配置，未执行真实 QQ 发送。
