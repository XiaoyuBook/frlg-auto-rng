...
class PySidePreviewInteractionTests(unittest.TestCase):
    ...
    def test_advanced_settings_are_reachable_and_keep_manual_values(self):
        ...
        self.assertTrue(window.advanced_button.isVisible())
        ...
        self.assertFalse(window.fields["expansion_3_adv"].isVisible())
        titles = [button.text() for button in window.reverse_config_buttons]
        self.assertEqual(titles, ["野生 / 定点反查", "波克比 Seed 反查", "孵蛋 Seed 反查"])
        window.reverse_config_buttons[0].click()
        self.assertTrue(window.fields["expansion_3_adv"].isVisible())
        window.fields["expansion_3_adv"].setText("12345")
        window.reverse_config_dialogs["wild"].hide()
        window.reverse_config_buttons[1].click()
        self.assertTrue(window.fields["togepi_reverse_adv"].isVisible())
        window.reverse_config_dialogs["togepi"].hide()
        window.reverse_config_buttons[2].click()
        self.assertTrue(window.fields["egg_reverse_seed"].isVisible())
        window.reverse_config_dialogs["egg"].hide()
