"""Standalone QQ Open Platform client using Qt's asynchronous network APIs.
...
class QQClient(QObject):
    ...
    def __init__(self, parent=None, *, api_base="https://api.sgroup.qq.com",
        ...
        self._deadline.timeout.connect(lambda: self._gateway_fail("绑定超时，请重新绑定并发送当前验证码。"))
    ...
    def _connect_gateway(self, gateway):
        ...
        self._code = str(secrets.randbelow(900000) + 100000)
    ...
    def _gateway_message(self, payload, token):
        ...
        if op == 10:
            ...
            if event == "READY":
                self._ready = True
                instruction = (f"请在 60 秒内私聊机器人，发送验证码：{self._code}" if self._kind == "user"
                               else f"请在 60 秒内在目标群 @机器人，发送验证码：{self._code}")
                self._deadline.start(self.bind_timeout_ms)
                self.status.emit(instruction)
                self.log.emit(instruction)
                return
            if not self._ready:
                return
